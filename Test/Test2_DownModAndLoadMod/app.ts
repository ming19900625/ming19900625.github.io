...
class Program {
    ...
    private fragInput: HTMLInputElement;
    private drawCylinderBtn: HTMLButtonElement;
    private downloadBtn: HTMLButtonElement;
    private uploadFilesInput: HTMLInputElement;
    private drawModelBtn: HTMLButtonElement;

    constructor() {
        this.fragInput = <HTMLInputElement>document.getElementById("fragInput");
        this.drawCylinderBtn = <HTMLButtonElement>document.getElementById("drawCylinderBtn");
        this.drawCylinderBtn.onclick = () => {
            let fragNum: number = parseInt(this.fragInput.value);
            this.cylinder = new Cylinder(fragNum,2);
            this.drawCylinder(this.cylinder);
        }
        this.downloadBtn = <HTMLButtonElement>document.getElementById("downloadBtn");
        ...
        this.uploadFilesInput = <HTMLInputElement>document.getElementById("file");
        this.drawModelBtn = <HTMLButtonElement>document.getElementById("drawModelBtn");
        this.drawModelBtn.onclick = () => {
            this.upload();
        }
    }
    drawCylinder = (cylinder: Cylinder) => {
        ...
        //var seg: number = 8;
        //let cylinder: Cylinder = new Cylinder(num, 3);
        ...
        // 视图变换坐标矩阵  
        //m.lookAt([-4, -2, -2], [0, 0, 0], [0, 1, 0], vMatrix);
        ...
        image.onload = function () {
            ...
            gl.drawElements(gl.TRIANGLES, cylinder.getDrawCount, gl.UNSIGNED_SHORT, 0);
    ...
    download = () => {
        let materialName: string = "test.mtl";
        let picName: string = "11.jpg";
        var image = new Image();
        image.src = '../images/11.jpg';

        FileHelper.SaveFile(picName, '../images/11.jpg');
        FileHelper.SaveStringFile(materialName, this.cylinder.getMaterialStr(picName));
        FileHelper.SaveStringFile("cylinder.obj", this.cylinder.getString(materialName));
    }
    ...
    upload = () => {
        var that = this;
        var files: FileList = this.uploadFilesInput.files;
        for (let i = 0; i < files.length; i++) {
            ...
            if (file.name.indexOf(".obj") >= 0) {
                ...
                reader.onload = function () {
                    console.log("obj:" + this.result);
                    that.mod = new Model(this.result);
                    if (that.mod != null) {
                        //that.drawModel(that.mod, that.imgSrc);
                        that.tryDraw();
                    }
                }
            }
            if (file.name.indexOf(".mtl") >= 0) {
                ...
                reader.readAsText(file);
                reader.onload = function () {
                    console.log("mtl:" + this.result);
                }
            }
            if (file.name.toLowerCase().indexOf(".png") >= 0 || file.name.toLowerCase().indexOf(".jpg") >= 0) {
    ...
    tryDraw() {
        if (this.mod == null)
            return;
        this.drawModel(this.mod, this.imgSrc);
    }

    drawModel = (model: Model,imgPath:string) => {
        var testVertexCode = '\
            attribute vec3 aVertexPosition;\
            attribute vec2 mp;\
            attribute vec3 normal;\
    ...
    //texture2D(tex,mp_v)*v_Color+v_AmbientColor;\
        var canvas = <HTMLCanvasElement>document.getElementById("renderCanvas");
        canvas.width = 800;
        canvas.height = 800;
        var gl: WebGLRenderingContext = canvas.getContext("webgl") || canvas.getContext("experimental-webgl");
        ...
        var cubeVerticesTextureCoordBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, cubeVerticesTextureCoordBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, model.getUVs, gl.STATIC_DRAW);
        ...
        var cubeVerticesIndexBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, cubeVerticesIndexBuffer);
        gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, model.getVertexIndexs, gl.STATIC_DRAW);

        //定义顶点数据
        var cubeVerticesBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, cubeVerticesBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, model.getVertexs, gl.STATIC_DRAW);
        var vertexPositionAttribute = gl.getAttribLocation(program, 'aVertexPosition');
        gl.enableVertexAttribArray(vertexPositionAttribute);
        gl.vertexAttribPointer(vertexPositionAttribute, 3, gl.FLOAT, false, 0, 0);
        ...
        var cubeNormalsBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, cubeNormalsBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, model.getNormals, gl.STATIC_DRAW);
        var vertexNormalAttribute = gl.getAttribLocation(program, 'normal');
        gl.enableVertexAttribArray(vertexNormalAttribute);
        gl.vertexAttribPointer(vertexNormalAttribute, 3, gl.FLOAT, false, 0, 0);
        ...
        // 视图变换坐标矩阵  
        //m.lookAt([-4, -2, -2], [0, 0, 0], [0, 1, 0], vMatrix);
        m.lookAt([2, 0, 2], [0, 0, 0], [0, 1, 0], vMatrix);
        // 投影坐标变换矩阵  
        m.perspective(90, canvas.width / canvas.height, 0.1, 100, pMatrix);
        ...
        image.onload = function () {
            ...
            gl.drawElements(gl.TRIANGLES, model.getDrawCount, gl.UNSIGNED_SHORT, 0);
            //gl.drawElements(gl.LINE_LOOP, seg * 12, gl.UNSIGNED_SHORT, 0);
            // context的刷新  
            gl.flush();
        };
